abstract class Casilla<F, V> {
  private vecinos: Map<F, Casilla> = null;
  ...
  addVecino(lado: F, vecino: Casilla) : void{
  ...
  addValor(valor: V){
    ...
    for (let key of this.vecinos.keys()) {
      if(this.iguales(this.valor, this.vecinos.get(key).getValor())){
        contador = contador + 1;
        iguales.push(key);
      }
    }
